...
export class ProductsService {
  ...
  if (!createProductDto.images || createProductDto.images.length < 2 || createProductDto.images.length > 5) {
  ...
  const savedProduct = await this._dataSource.transaction(async (manager) => {
    ...
    const subCategory = await manager.findOne(SubCategory, { where: { id: createProductDto.subCategoryId } });
    if (!subCategory) {
    ...
    const product = manager.create(Product, {
      ...
      discountStartDate: createProductDto.discountStartDate ? new Date(createProductDto.discountStartDate) : null,
...
async findAll(filters: ProductFilters = {}) {
  ...
  if (filters.subCategoryId)
    queryBuilder.andWhere('product.subCategoryId = :subCategoryId', { subCategoryId: filters.subCategoryId });
  if (filters.targetedGender)
    queryBuilder.andWhere('product.targetedGender = :targetedGender', { targetedGender: filters.targetedGender });
  
  // Price filter using join (more efficient than subquery)
  if (filters.minPrice !== undefined) queryBuilder.andWhere('variant.price >= :minPrice', { minPrice: filters.minPrice });
  if (filters.maxPrice !== undefined) queryBuilder.andWhere('variant.price <= :maxPrice', { maxPrice: filters.maxPrice });
  ...
  switch (errorCode) {
    ...
    case 'ER_DUP_ENTRY':
      ...
      throw new ConflictException(
        `Duplicate value for ${field}. This value must be unique.`
      );
    ...
    //   this.metrics.increment('db.deadlocks');
      throw new ServiceUnavailableException(
        'Temporary database conflict. Please retry the request.'
      );
