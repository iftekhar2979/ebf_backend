...
private readonly redisService: RedisService,
    ...
    private productBoostingRepository: Repository<ProductBoosting>,
  ...
  private calculateTrendingScore(
    ...
    const boostMultiplier = boost && this.isBoostActive(boost) 
      ? this.weights.boostMultiplier 
      : 1.0;
  ...
  async updateProductRanking(productId: number): Promise<void> {
    try {
      ...
      if (!product) {
        this.logger.warn(`Product ${productId} not found for ranking update`);
        return;
      }
      ...
      const score = this.calculateTrendingScore(
        ...
        await this.redisService.zadd(this.TRENDING_KEY, score, productId.toString());
        ...
        if (this.hasActiveDiscount(product)) {
          ...
          await this.redisService.zadd(this.DISCOUNTED_KEY, discountScore, productId.toString());
        
        } else {
          // Remove from discounted if discount expired
          await this.redisService.zdel(this.DISCOUNTED_KEY, productId.toString());
        }

        // Add to new arrivals if less than 30 days old
        if (createdDaysAgo <= 30) {
          await this.redisService.zadd(this.NEW_ARRIVALS_KEY, Date.now(), productId.toString());
        }

        this.logger.debug(`Updated ranking for product ${productId} with score ${score}`);
      
    } catch (error) {
      this.logger.error(`Failed to update ranking for product  ${error.message}`);
    }
  }
  ...
  async getTrendingProducts(limit: number = 20, offset: number = 0): Promise<number[]> {
    try {
        ...
        const results = await this.redisService.zrange(this.TRENDING_KEY, offset, offset + limit - 1);
  ...
  async getDiscountedProducts(limit: number = 20, offset: number = 0): Promise<number[]> {
    try {
      ...
      this.logger.error(`Failed to get discounted products: ${error.message}`);
      return [];
    }
  }
  ...
  async getNewArrivals(limit: number = 20, offset: number = 0): Promise<number[]> {
    try {
      ...
      this.logger.error(`Failed to get new arrivals: ${error.message}`);
      return [];
    }
  }
  ...
  async rebuildAllRankings(): Promise<void> {
    this.logger.log('Starting full ranking rebuild...');
    ...
    try {
      ...
      this.logger.log(`Completed ranking rebuild for ${productIds.length} products`);
    } catch (error) {
      this.logger.error(`Failed to rebuild rankings: ${error.message}`);
    }
  }
  ...
  async cleanupOldRankings(): Promise<void> {
    try {
        ...
        await this.redisService.zRemoveRangeByScore(this.NEW_ARRIVALS_KEY, 0, thirtyDaysAgo);
        
        // Remove products with zero score from trending
        await this.redisService.zRemoveRangeByScore(this.TRENDING_KEY, 0, 0);
        
        this.logger.log('Completed ranking cleanup');
      
    } catch (error) {
      this.logger.error(`Failed to cleanup rankings: ${error.message}`);
    }
  }
  ...
  async getProductTrendingRank(productId: number): Promise<number | null> {
    try {
        ...
        const rank = await this.redisService.zRevRank(this.TRENDING_KEY, productId.toString());
        return rank !== null ? rank + 1 : null; // Convert to 1-based ranking
      
      
    } catch (error) {
      this.logger.error(`Failed to get product rank: ${error.message}`);
      return null;
    }
  }
  ...
  async incrementViewsAndUpdateRanking(productId: number): Promise<void> {
    // Update stats
    await this.productStatRepository.increment(
      { productId },
      'viewCount',
      1,
    );
  ...
  async removeProductFromRankings(productId: number): Promise<void> {
    try {
        ...
        await Promise.all([
          this.redisService.zdel(this.TRENDING_KEY, productId.toString()),
          this.redisService.zdel(this.DISCOUNTED_KEY, productId.toString()),
          this.redisService.zdel(this.NEW_ARRIVALS_KEY, productId.toString()),
          this.redisService.zdel(this.POPULAR_KEY, productId.toString()),
        ]);
      
    } catch (error) {
      this.logger.error(`Failed to remove product from rankings: ${error.message}`);
    }
  }
}